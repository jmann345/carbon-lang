...
- `COMPILE-ARGS` is whitespace-split and inserted into the `carbon compile`
  ...
  compile configuration — e.g. `--cpp-exceptions=none` for the error-handling
  boundary programs. A program whose flags the driver rejects is an honest
  `COMPILE-FAIL` (write-tests-first: red until the flag lands).
