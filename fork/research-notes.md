...
2.  **Write the rulebook.** How code is to be translated/written: idioms,
    type mappings, error-handling conventions, what to refactor versus
    translate mechanically. The rulebook comes _before_ the gap inventory.
...
4.  **Create the gap list.** An inventory of places needing judgment
    ...
    rulebook's defaults _won't_ cover. Rulebook + gap list are stress-tested
    together in a joint audit.
5.  **Trial run ("shakedown cruise").** A mini-migration before scaling:
    for example one agent translates 3 files by the rulebook, another translates
...
million lines of unreviewed slop?" — that is **an arbiter is only as good as
its coverage**, and a process that outruns reflection accumulates debt.
Consequences for this fork:

-   Grow the arbiter _first and continuously_: every feature workstream
...
## 3. Why Carbon-to-0.1 is _not_ a migration, and what transfers
...
-   The **rulebook** → Carbon already has one: `docs/design/`, the accepted
    ...
    _design gaps_ — exactly the "gap list" of places needing human judgment
    (= the user's design-fork decision points).
-   The **dependency map** → feature dependency ordering (for example sum types
...
Key difference: milestone bullets need _design decisions_ first, and per
