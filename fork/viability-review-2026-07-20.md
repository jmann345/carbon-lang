...
merged-slice/day with 2 in flight, 55–105 slices fits — *if* 1.5–2
...
fork-ratified designs) by end of 2026 is *plausible but fragile* — call
...
Read: toolchain/check/handle_match.cpp @ HEAD (merged via 5a83b0e).
...
- **F-Q1: parse-tree index arithmetic.** handle_match.cpp:106-111 sniffs
  ...
  pattern-matching context" — i.e., it dodges integrating with the
...
negative *scrutinee*, match_switch.carbon:47).
...
- Mitigation 1: the merge gate runs upstream's full 1,625-golden suite
  (F-002, decision-log.md:199-213), which catches *regressions* to
...
- **Hole (F-A1): for NEW fork features the goldens are generated by the
  implementation under test** via runner-side autoupdate (R15). R16(d)
...
- **Decision load: 51 user ratifications on day 1** (decision-log.md:
  ...
  of the recommendation is the signature of rubber-stamping, i.e., the
...
- **R-S2 Orchestrator context compaction.** Mitigated unusually well
  ...
  2026-07-19T07:23 against the *upstream nightly*, 60 PASS, pre-merge —
...
9. **Record user-visible slice gaps as inventory items at merge time**
   (e.g. `case -1`): a gap a user can hit must exist in
