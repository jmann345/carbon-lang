...
-   **R8. Reviewer findings must cite the rule, design doc, proposal, or
    repository file behind them.** A finding with no citation is itself a finding.
...
-   **R12. Mechanical invariants are enforced by PostToolUse hooks, not
    ...
    _mechanical_ class of error, the fix is extending the hook script, then
...
-   **R16. Passing tests is necessary, never sufficient — and gaming them
    ...
    CHECK lines in `toolchain/**/testdata/**` — goldens change ONLY by way of the
