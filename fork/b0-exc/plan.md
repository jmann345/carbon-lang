...
- **Testdata knobs.** file_test supports `// EXTRA-ARGS:` on check/lower
  goldens (e.g. `--clang-arg=-fno-exceptions` in
...
opt-level flag, which is *prepended* at .cpp:483-488 precisely so user args
...
(`__clang_call_terminate` → `std::terminate`), i.e. the C++-standard
...
(thunk.cpp:37). Applying it to *every* thunk in catch mode (not just
...
- `fail_throw_in_none.carbon` — `EXTRA-ARGS: --cpp-exceptions=none` with
  ...
  disabled" surfaces via CppInteropDriverError-style plumbing (locks
...
- `cpp_exceptions_fence_terminate.carbon` — `COMPILE-ARGS:
  ...
  call site never sees the exception; `EXPECT-EXIT: -6` (SIGABRT via
...
(e.g. by keeping all B0 programs on EXPECT-only status under a new bullet
