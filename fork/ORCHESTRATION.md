...
- `Fork: build toolchain` — full gate: tarball + `bazel test //toolchain/...`
  + release. Per-ref concurrency. Dispatch via workflow_dispatch on any ref.
...
- `Fork: autoupdate testdata` — regenerates goldens on the runner and pushes
  back (R15). Fire via push to `fork/autoupdate-request.txt` or dispatch.
...
- Sub-forks ALWAYS via AskUserQuestion (process.md Human-in-the-loop).
- Fixer is always a separate agent (R11); mechanical invariants via hooks
