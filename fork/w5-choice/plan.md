...
- Payload alternatives are **function-like**: constructed by "calling" them
  ...
  *surface* is "as today", but the constant's **value synthesis is not**:
...
- The contract operates at the compiler layout level; source-level union
  ...
  slices below nevertheless *restrict* payloads to trivially copyable +
...
compare against alternative indices, branch, extract payload via
element access on the overlapping storage. The `Match`
interface/Continuation mechanism in sum_types.md:124-246 is the
*user-defined sum type* generality and is **explicitly out of scope for the
...
*payload-free alternative patterns over a choice scrutinee* (`case .Err`),
...
CARBON_FATAL), which is deferred to slice 2 where payload *values* become
observable.
...
2. Widening the `MatchCaseIntroducer` lookahead gate
   ...
   shape.
3. Resolving `.Err` in case-pattern position against the *scrutinee type*.
...
| S2 | Payload destructuring: `case .Ok(value: i32)` binding patterns | L (XL risk) | Payload value round-trips: construct with runtime-computed argument, extract via match binding, print it. Differential pair vs C++ `std::variant` + `std::get_if`. Exhaustive alternative coverage replaces `default` (per SF-7 outcome). |
...
- `AddChoiceAlternative`: instead of the :158-162 rejection, queue the
  NameComponent *with* its param patterns (already fully pattern-checked —
...
- At `ChoiceDefinitionId`, **(a) payload alternatives**: synthesize a
  ...
  reads already work via convert.cpp:551-559; **writes/init are the new
...
- **(b) Payload-free alternative constants of payload-carrying choices**
  ...
  the discriminant and converts via ConvertStructToClass; that dies once
...
- Object repr: extend the :248-289 computation — collect payload tuple
  types, compute max size/align via the same completion-time queries the
...
- Discriminant scheme unchanged: `UInt(ceil(log2(n)))`, `()` for 0/1
  ...
  *with* a payload gets repr `{ .payload: CustomLayoutType }` (zero-bit
...
- H9 `MatchCase` (choice path): resolve the designator name in the
  scrutinee choice's class scope (via LoadImportRef so imported choices
...
- Conformance: new `types/choice_payload_construct.carbon` (un-SKIPped at
  ...
  `std::variant<int, std::monostate>` probed via `index()`, printf
...
1. **Parse**: `case .Ok(value: i32)` has no parse today —
   ...
   a paren *pattern* list (reusing the existing TuplePattern/paren-pattern
   states). New parse node kind(s) (e.g. `AlternativePattern`,
...
2. **Check pattern context**: case-arm bindings currently CARBON_FATAL
   ...
   (MatchHandlerStart's scope, W4 H12) via the existing BindName machinery,
...
- **SF-1 (blocks S1): object representation.** Shared cost first, so the
  ...
  a new inst kind + emitter, i.e. the §6 XL trigger fired on day one.
  **Recommend (a)**; both satisfy F-007k's letter ("using the same
  explicit-layout object representation" for the *overlapping storage*).
...
- **SF-3 (blocks S1): `Alt()` empty parameter list.** (a) Becomes a
  zero-payload *function-like* alternative — constructed `C.Alt()`,
...
- **SF-5 (blocks S2): binding spelling inside alternative patterns.**
  ...
  `let` rejected via the normal pattern grammar — revisit alongside F-011
...
- **SF-8 (blocks S4): std::variant mapping target.**
  ...
  index-named patterns; (b) maps only via user-declared
...
*context*, not new *kinds*) for S2; `BranchIf`/`Branch`/EqWith `Call` for
...
region cannot be expressed with existing init/access insts (e.g. an
...
- Every TODO string in §2.1 is contract text: conformance SKIP reasons
  ...
  co-change.
- clang-format 21.1.8 via hooks (R12/R18); `runner.py --self-test` before
...
- **R-4. Temporary/cleanup soundness when the scrutinee gate widens
  ...
  and the gate must be *type-property-based* (trivial destructibility),
...
- **R-5. custom_witness.cpp CustomLayoutType FATAL.** A native type with a
  ...
  S1 must add the case *before* the first native choice with a payload
...
- **R-11. No local build.** All goldens via runner-side autoupdate; plan
