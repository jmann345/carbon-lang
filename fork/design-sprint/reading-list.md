...
and steer the agents. Per fork: the in-repo sources first (highest
...
- Concept to understand: **unwinding vs. return-value propagation**, and
...
- Concept: **object representation vs. active member**, and why
...
- Hans Boehm, "Threads Cannot Be Implemented As a Library" — why the
  memory model must be a *language* property; explains what our design
  doc has to state.
...
these: in-repo authority first, then the minimum external reading that
