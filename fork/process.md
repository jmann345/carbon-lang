...
`research-notes.md`) to a _language-completion_ project. The unit of work
...
-   A **grader script** that runs the whole suite and emits a scoreboard:
    `bullet → PASS / FAIL / NOT-WRITTEN`. The scoreboard percentage _is_
...
The suite is written _before or with_ each feature, never after. This is
...
_agent_ parallelism (many readers/writers), while _build/test_
...
every struct/function to port. This fork's equivalents, all in-repository:
...
2.  **`fork/inventory/work-items.json`** — the work inventory: every known
    ...
    vibes; discovering un-inventoried work means _adding the item first_.
...
(`fork/design-sprint/reading-list.md` format): in-repository sources first,
...
3.  **Rulebook R16/R17**: the anti-Goodhart prohibitions (goldens only by way of
